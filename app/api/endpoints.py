...
def _normalize_recommendation_cards(
    rows: Any,
    max_items: int = 3,
) -> List[Dict[str, Any]]:
    ...
    for item in rows:
        ...
        evidence_items = _normalize_short_text_list(
            item.get("evidence_items") or item.get("evidenceItems") or evidence,
            max_items=3,
            max_chars=100,
        )
        action_steps = _normalize_short_text_list(
            item.get("action_steps") or item.get("actionSteps") or action,
            max_items=2,
            max_chars=120,
        )
        focus_preview = str(
            item.get("focus_preview") or item.get("focusPreview") or note or ""
        ).strip()
        target_metric = str(
            item.get("target_metric") or item.get("targetMetric") or target or ""
        ).strip()
        ...
        normalized.append(
            {
                ...
                "focus_preview": focus_preview[:220],
                "target_metric": target_metric[:180],
                "cta_type": cta_type[:80],
            }
        )
...
def _build_student_analysis_fallback(
    payload: StudentAnalysisSummaryRequest,
) -> Dict[str, Any]:
    ...
    patterns = (
        payload.priority_patterns if isinstance(payload.priority_patterns, dict) else {}
    )
    available_actions = [
        item
        for item in (payload.available_actions or [])
    ...
    confident_wrong_rate = max(
        0, min(100, _coerce_int(metrics.get("confident_wrong_rate_pct"), 0))
    )

    trend_label = (
        str(trend.get("direction_label") or "ยังทรงตัว").strip() or "ยังทรงตัว"
    )
    ...
    latest_wrong_questions = max(
        0, _coerce_int(patterns.get("latest_wrong_questions"), 0)
    )
    confident_wrong_questions = max(
        0, _coerce_int(patterns.get("confident_wrong_questions"), 0)
    )
    top_questions = [
        item for item in (patterns.get("top_questions") or []) if isinstance(item, dict)
    ]
    weak_topics = [
        item for item in (patterns.get("weak_topics") or []) if isinstance(item, dict)
    ]
    ...
    top_question_recent_wrong = max(
        0, _coerce_int(top_question.get("recent_wrong_count"), 0)
    )
    ...
    weak_topic_question_count = max(
        0, _coerce_int(top_weak_topic.get("question_count"), 0)
    )
    trend_delta_text = (
        f" ({'+' if trend_delta > 0 else ''}{trend_delta}%)" if trend_delta != 0 else ""
    )
    trend_line = (
        f"คะแนนช่วงล่าสุด{trend_label}{trend_delta_text} โดยค่าเฉลี่ยอยู่ที่ {avg_score}% "
        f"และคะแนนล่าสุด {last_score}% (Accuracy 7 วัน {accuracy7d}%)"
    )

    trend_title = (
        "แนวโน้มกำลังดีขึ้น"
        if trend_delta > 0
        else ("แนวโน้มช่วงล่าสุดลดลง" if trend_delta < 0 else "คะแนนยังทรงตัว")
    )
    trend_action_item = pick_action("practice_recent_mistakes", "practice_weak_topic")
    trend_action = (
        f"ภายใน 24 ชั่วโมง ทำชุด {action_label(trend_action_item, 'ฝึกซ้ำ 10 ข้อที่พลาดล่าสุด')} แล้วทบทวนข้อที่ยังผิดซ้ำ"
        if trend_delta < 0
        else (
            f"ทำชุด {action_label(trend_action_item, 'ฝึกซ้ำ 10 ข้อที่พลาดล่าสุด')} 1 รอบ เพื่อรักษาจังหวะและดัน Accuracy 7 วันให้ถึง {max(70, min(95, accuracy7d + 3))}%"
            if trend_delta > 0
            else f"ทำชุด {action_label(trend_action_item, 'ฝึกซ้ำ 10 ข้อที่พลาดล่าสุด')} 1 รอบ แล้วเช็กเฉลยเฉพาะข้อที่ยังพลาด"
        )
    )

    speed_target = max(20, int(round(avg_sec * 0.9))) if avg_sec > 0 else 30
    advice_line = (
        f"รอบถัดไปตั้งเป้า Accuracy 7 วันอย่างน้อย {max(70, accuracy7d)}% และคุมเวลาเฉลี่ยไม่เกิน {speed_target} วินาที/ข้อ"
        if avg_sec > 0
        else f"รอบถัดไปตั้งเป้า Accuracy 7 วันอย่างน้อย {max(70, accuracy7d)}%"
    )
    if sampled_questions > 0:
        advice_line += (
            f" พร้อมทบทวนข้อที่พลาด {wrong_questions}/{sampled_questions} ข้อ"
        )
    ...
    if top_question_label:
        summary_paragraph = (
            f"{top_question_label}"
            f"{f' หัวข้อ {top_question_topic}' if top_question_topic else ''}"
            f" เป็นจุดที่ควรแก้ก่อน เพราะผิด {top_question_wrong} ครั้ง"
            f" จากข้อมูล {attempts} ครั้งล่าสุด"
        )
    else:
    ...
    focus_action = (
        f"วันนี้ทำชุด {action_label(focus_action_item, 'เก็บหัวข้ออ่อน 10 ข้อ')} แล้วจดสาเหตุทุกข้อที่ผิด"
        if focus_label
        else f"วันนี้ทำชุด {action_label(focus_action_item, 'ฝึกซ้ำ 10 ข้อที่พลาดล่าสุด')} แล้วทำซ้ำให้ถูกครบ"
    )
    confidence_note = (
        f"พบมั่นใจแต่ตอบผิด {confident_wrong_questions} ข้อ ควรเช็กวิธีคิดก่อนกดตอบ"
        if confident_wrong_questions > 0
        else ""
    )
    level_up_action_item = pick_action(
        "practice_level_up", "practice_weak_topic", "practice_recent_mistakes"
    )
    if top_question_label:
        question_evidence_parts = [
            f"ผิดสะสม {top_question_wrong} ครั้ง",
            f"พลาดใน 5 ครั้งล่าสุด {top_question_recent_wrong} ครั้ง"
            if top_question_recent_wrong > 0
            else "",
            f"เฉลี่ย {top_question_avg_sec} วิ/ข้อ" if top_question_avg_sec > 0 else "",
            f'"{top_question_preview[:90]}"' if top_question_preview else "",
        ]
        question_evidence = " | ".join(part for part in question_evidence_parts if part)
        recommendation_cards = [
            {
                "title": f"แก้ข้อผิดซ้ำ: {top_question_label}",
                "evidence": question_evidence,
                "action": (
                    f"ทำชุด {action_label(trend_action_item, 'ฝึกซ้ำ 10 ข้อที่พลาดล่าสุด')} "
                    f"แล้วอ่านเฉลยของ {top_question_label} ก่อนทำซ้ำ"
                ),
                "note": (
                    f"หัวข้อ {top_question_topic} ต้องตอบถูกติดกันก่อนข้าม"
                    if top_question_topic
                    else "เช็กวิธีคิดก่อนดูเฉลย"
                ),
                "target": (
                    f"ตอบ {top_question_label} และข้อแนวเดียวกันให้ถูก 2 รอบติด"
                ),
                "evidence_items": [part for part in question_evidence_parts if part][
                    :4
                ],
                "action_steps": [
                    f"ทำชุด {action_label(trend_action_item, 'ฝึกซ้ำ 10 ข้อที่พลาดล่าสุด')}",
                    f"อ่านเฉลยของ {top_question_label} แล้วสรุปว่าพลาดตรงไหน",
                    f"ทำ {top_question_label} ซ้ำจนตอบถูกติดกัน",
                ],
                "focus_preview": (
                    f"{top_question_label}: {top_question_preview}"
                    if top_question_preview
                    else top_question_label
                ),
                "target_metric": (
                    f"ถูก 2 รอบติดใน {top_question_label} และข้อแนวเดียวกัน"
                ),
                "cta_type": str(trend_action_item.get("type") or ""),
            },
            {
                "title": (
                    f"เก็บหัวข้อ {weak_topic_label}"
                    if weak_topic_label
                    else f"เก็บข้อที่พลาดล่าสุด {latest_wrong_questions} ข้อ"
                ),
                "evidence": (
                    f"หัวข้อนี้ผิดรวม {weak_topic_wrong} ครั้ง จาก {weak_topic_question_count} ข้อที่ระบบจับได้"
                    if weak_topic_wrong > 0
                    else improvement_line
                ),
                "action": focus_action,
                "note": focus_note or confidence_note,
                "target": (
                    f"ลดข้อผิดหัวข้อนี้ให้เหลือไม่เกิน {max(1, int(weak_topic_wrong * 0.7))} ครั้ง"
                    if weak_topic_wrong > 0
                    else f"ลดข้อผิดใน{focus_prefix}นี้ลงอย่างน้อย 30%"
                ),
                "evidence_items": [
                    f"ผิดรวม {weak_topic_wrong} ครั้ง"
                    if weak_topic_wrong > 0
                    else improvement_line,
                    f"{weak_topic_question_count} ข้อที่ระบบจับได้"
                    if weak_topic_question_count > 0
                    else "",
                    f"หัวข้อ {weak_topic_label}" if weak_topic_label else "",
                ],
                "action_steps": [
                    f"ทำชุด {action_label(focus_action_item, 'เก็บหัวข้ออ่อน 10 ข้อ')}",
                    "จดสาเหตุข้อที่ผิดเป็นคำสั้นๆ หลังดูเฉลย",
                    "ทำซ้ำเฉพาะข้อที่สาเหตุซ้ำกัน",
                ],
                "focus_preview": (
                    f"{weak_topic_label}: เริ่มจาก {top_question_label}"
                    if weak_topic_label and top_question_label
                    else weak_topic_label
                ),
                "target_metric": (
                    f"ข้อผิดหัวข้อนี้เหลือไม่เกิน {max(1, int(weak_topic_wrong * 0.7))} ครั้ง"
                    if weak_topic_wrong > 0
                    else f"ลดข้อผิดใน{focus_prefix}นี้ลงอย่างน้อย 30%"
                ),
                "cta_type": str(focus_action_item.get("type") or ""),
            },
            {
                "title": "วัดผลรอบถัดไป",
                "evidence": (
                    f"ล่าสุด {last_score}% | Accuracy 7 วัน {accuracy7d}% | พลาดล่าสุด {latest_wrong_questions} ข้อ"
                ),
                "action": (
                    f"ทำชุด {action_label(level_up_action_item, 'รวมโจทย์ท้าทาย')} หลังทบทวนข้อผิดซ้ำ"
                ),
                "note": (
                    f"คุมเวลาเฉลี่ยต่อข้อไม่เกิน {speed_target} วินาที"
                    if avg_sec > 0
                    else "ถ้าผิดซ้ำให้กลับไปอ่านเฉลยข้อแรกก่อน"
                ),
                "target": advice_line,
                "evidence_items": [
                    f"ล่าสุด {last_score}%",
                    f"Accuracy 7 วัน {accuracy7d}%",
                    f"พลาดล่าสุด {latest_wrong_questions} ข้อ",
                ],
                "action_steps": [
                    "ทบทวนข้อผิดซ้ำก่อนเริ่มจับเวลา",
                    f"ทำชุด {action_label(level_up_action_item, 'รวมโจทย์ท้าทาย')}",
                    "หลังทำเสร็จ ให้เทียบว่าข้อผิดซ้ำลดลงหรือไม่",
                ],
                "focus_preview": "เริ่มจากข้อผิดซ้ำก่อน แล้วค่อยทำโจทย์ท้าทาย",
                "target_metric": advice_line,
                "cta_type": str(level_up_action_item.get("type") or ""),
            },
        ]
    else:
        recommendation_cards = [
            {
                ...
                "action": trend_action,
                "note": (
                    f"คะแนนเฉลี่ย {avg_score}% | ล่าสุด {last_score}% | Accuracy 7 วัน {accuracy7d}%"
                ),
                "target": f"Accuracy 7 วันอย่างน้อย {max(70, accuracy7d)}%",
                "evidence_items": [
                    f"เฉลี่ย {avg_score}%",
                    f"ล่าสุด {last_score}%",
                    f"Accuracy 7 วัน {accuracy7d}%",
                ...
                "action_steps": [
                    f"ทำชุด {action_label(trend_action_item, 'ฝึกซ้ำ 10 ข้อที่พลาดล่าสุด')}",
                    "เช็กเฉลยเฉพาะข้อที่ยังพลาด",
                    "กลับมาดูแนวโน้มหลังจบรอบ",
                ],
                "focus_preview": trend_line,
                "target_metric": f"Accuracy 7 วันอย่างน้อย {max(70, accuracy7d)}%",
                "cta_type": str(trend_action_item.get("type") or ""),
            },
            {
                "title": f"จุดอ่อนหลัก: {focus_target_label}",
                "evidence": improvement_line,
                "action": focus_action,
                "note": confidence_note or focus_note,
                "target": f"ลดข้อผิดใน{focus_prefix}นี้ลงอย่างน้อย 30%",
                "evidence_items": [improvement_line, confidence_note or focus_note],
                "action_steps": [
                    f"ทำชุด {action_label(focus_action_item, 'เก็บหัวข้ออ่อน 10 ข้อ')}",
                    "จดสาเหตุทุกข้อที่ผิด",
                    "ทำซ้ำข้อที่สาเหตุซ้ำกัน",
                ],
                "focus_preview": focus_target_label,
                "target_metric": f"ลดข้อผิดใน{focus_prefix}นี้ลงอย่างน้อย 30%",
                "cta_type": str(focus_action_item.get("type") or ""),
            },
            {
                "title": "เป้าหมายรอบถัดไป",
                "evidence": (
                    f"พลาด {wrong_questions}/{sampled_questions} ข้อจากข้อมูลที่วิเคราะห์"
                    if sampled_questions > 0
                    else "ยังมีข้อมูลรายข้อไม่มาก แนะนำเก็บรอบฝึกเพิ่มเพื่อวิเคราะห์ให้แม่นขึ้น"
                ),
                "action": f"ทำชุด {action_label(level_up_action_item, 'รวมโจทย์ท้าทาย')} แล้วตั้งเป้าตามผลรอบถัดไป",
                "note": (
                    f"คุมเวลาเฉลี่ยต่อข้อไม่เกิน {speed_target} วินาที"
                    if avg_sec > 0
                    else "รักษาความสม่ำเสมอในการฝึกต่อเนื่อง"
                ),
                "target": advice_line,
                "evidence_items": [
                    f"พลาด {wrong_questions}/{sampled_questions} ข้อ"
                    if sampled_questions > 0
                    else "ยังมีข้อมูลรายข้อไม่มาก",
                    f"เวลาเป้า {speed_target} วิ/ข้อ" if avg_sec > 0 else "",
                ],
                "action_steps": [
                    f"ทำชุด {action_label(level_up_action_item, 'รวมโจทย์ท้าทาย')}",
                    "ตรวจข้อผิดทันทีหลังทำ",
                    "ตั้งเป้าตามผลรอบถัดไป",
                ],
                "focus_preview": "ใช้รอบนี้เพื่อแยกสาเหตุข้อผิดให้ชัดขึ้น",
                "target_metric": advice_line,
                "cta_type": str(level_up_action_item.get("type") or ""),
            },
        ]
    ...
    return {
        ...
        "recommendations": [trend_line, improvement_line, advice_line],
...
def _build_analysis_summary_cache_key(payload: StudentAnalysisSummaryRequest) -> str:
    normalized_payload = {
        "recommendation_schema_version": str(
            payload.recommendation_schema_version or ""
        ).strip(),
        "analysis_plan": payload.analysis_plan
        if isinstance(payload.analysis_plan, list)
        else [],
        "context": payload.context if isinstance(payload.context, dict) else {},
        "metrics": payload.metrics if isinstance(payload.metrics, dict) else {},
        "recent_trend": payload.recent_trend
        if isinstance(payload.recent_trend, dict)
        else {},
        "focus_area": payload.focus_area
        if isinstance(payload.focus_area, dict)
        else {},
        "priority_patterns": payload.priority_patterns
        if isinstance(payload.priority_patterns, dict)
        else {},
        "available_actions": payload.available_actions
        if isinstance(payload.available_actions, list)
        else [],
        "client_recommendation_cards": payload.client_recommendation_cards
        if isinstance(payload.client_recommendation_cards, list)
        else [],
    }
...
@router.post("/users/{user_id}/courses/{course_id}/analysis-summary")
async def generate_student_analysis_summary(
    user_id: str,
    course_id: str,
    payload: StudentAnalysisSummaryRequest = Body(...),
    credentials: Optional[HTTPAuthorizationCredentials] = Depends(
        STUDENT_BEARER_OPTIONAL
    ),
    student_auth_service: StudentAuthService = Depends(_get_student_auth_service),
    ...
    """Generate AI summary for student analysis using LiteLLM."""
    try:
        await _ensure_user_matches_token(
            user_id=user_id,
            credentials=credentials,
        ...
        settings = get_settings()
        resolved_model = str(settings.litellm_model).strip() or settings.litellm_model

        fallback = _build_student_analysis_fallback(payload)
        cache_key = _build_analysis_summary_cache_key(payload)
        score_version = await dynamodb_service.get_student_course_score_version(
            user_id, course_id
        )
        cached_response = await dynamodb_service.get_student_analysis_summary_cache(
            user_id=user_id,
            course_id=course_id,
            cache_key=cache_key,
        ...
        if isinstance(cached_response, dict):
            cached_cards = _normalize_recommendation_cards(
                cached_response.get("recommendation_cards")
            )
            if not cached_cards:
                cached_cards = _normalize_recommendation_cards(
                    fallback.get("recommendation_cards")
                )
            return {
                **cached_response,
                "recommendation_cards": cached_cards[:3],
                "cached": True,
            }

        default_plan = [
            "วิเคราะห์แนวโน้มคะแนน",
            "จุดที่ต้องปรับปรุง",
            "คำแนะนำ",
        ]
        normalized_plan = [
            str(item).strip()
            for item in (payload.analysis_plan or [])
        ...
        glossary_block = (
            "- context.analysis_source_type: ประเภทการวิเคราะห์ (lesson หรือ mock_exam)\n"
        ...
        prompt = (
            "คุณเป็นผู้ช่วยวิเคราะห์ผลการเรียนของนักเรียน\n"
            ...
            f"แผนการสรุป:\n{plan_block}\n\n"
            f"คำอธิบายความหมายของฟิลด์:\n{glossary_block}\n\n"
            f"ข้อมูลนักเรียน (JSON):\n{json.dumps(input_data, ensure_ascii=False)}"
        )

        litellm_user, litellm_metadata = await chat_service._get_litellm_user_context(
            user_id
        )
        raw_text = await chat_service._call_gemini_chat(
            prompt,
            max_tokens=900,
        ...
        summary_paragraph = str(
            ...
            or fallback.get("summary_paragraph")
            or ""
        ).strip()
        ...
        if isinstance(fallback_rows, list):
            fallback_recommendations = [
                str(item).strip() for item in fallback_rows if str(item or "").strip()
            ]
        fallback_cards = _normalize_recommendation_cards(
            fallback.get("recommendation_cards")
        )

        recommendation_cards = _normalize_recommendation_cards(
            parsed.get("recommendation_cards")
        )
        allowed_cta_types = {
        ...
        if len(recommendation_cards) < 3:
            seen_card_signatures = {
                ...
                for item in recommendation_cards
            }
        ...
        await dynamodb_service.set_student_analysis_summary_cache(
            user_id=user_id,
            course_id=course_id,
            cache_key=cache_key,
            score_version=score_version,
        ...
        response_payload = {
            ...
            "recommendations": [
                str(item).strip()
                for item in (fallback.get("recommendations") or [])
                if str(item or "").strip()
            ][:3],
            "recommendation_cards": _normalize_recommendation_cards(
                fallback.get("recommendation_cards")
            )[:3],
            "model": str(get_settings().litellm_model).strip()
            or get_settings().litellm_model,
            "is_fallback": True,
            "generated_at": datetime.utcnow().isoformat() + "Z",
            "cached": False,
        }
...
async def submit_quiz_answers(
    ...
    try:
        ...
        invalidate_summary_cache = getattr(
            dynamodb_service,
            "invalidate_student_analysis_summary_cache",
            None,
        )
        if effective_course_id and callable(invalidate_summary_cache):
            await invalidate_summary_cache(
                user_id=user_id, course_id=effective_course_id
            )
