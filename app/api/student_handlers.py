...
def _build_dashboard_course_stats(courses: List[Dict[str, Any]], lessons: List[Dict[str, Any]], quizzes: List[Dict[str, Any]], quiz_results: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    for course in courses:
        ...
        for lesson in sorted(lesson_buckets.values(), key=lambda row: (row.get('order') or 0, str(row.get('name') or ''))):
            ...
            completed_lesson_quizzes = len([quiz_id for quiz_id in lesson_quiz_set if quiz_id in attempted_quiz_ids])
            lesson_progress = round(completed_lesson_quizzes / total_lesson_quizzes * 100) if total_lesson_quizzes > 0 else None
            computed_lesson_rows.append({'id': lesson['id'], 'name': lesson['name'], 'scoreSplit': {'lesson': round(lesson['lesson']['correct'] / lesson_total * 100) if lesson_total > 0 else None, 'mockExam': round(lesson['mockExam']['correct'] / mock_total * 100) if mock_total > 0 else None}, 'minutes': round(lesson['minutes']) if lesson['minutes'] > 0 else 0, 'totalQuizzes': total_lesson_quizzes, 'completedQuizzes': completed_lesson_quizzes, 'progress': lesson_progress})
        attempt_rows = []
