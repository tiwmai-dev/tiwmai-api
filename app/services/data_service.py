...
class SupabaseDataService:
    ...
    def _analysis_summary_cache_entry_key(
        self, cache_key: Optional[str], score_version: Optional[str]
    ) -> Optional[str]:
        normalized_cache_key = str(cache_key or "").strip()
        if not normalized_cache_key:
            return None
        normalized_score_version = str(score_version or "").strip()
        return f"{normalized_score_version}:{normalized_cache_key}"

    async def get_student_analysis_summary_cache(
        self,
        user_id: str,
        course_id: str,
        ...
        cache = (user or {}).get("student_analysis_summary_cache") or {}
        course_cache = cache.get(course_id)
        entry_key = self._analysis_summary_cache_entry_key(cache_key, score_version)
        if not entry_key:
            return course_cache if isinstance(course_cache, dict) else None
        if not isinstance(course_cache, dict):
            return None
        entries = course_cache.get("entries")
        if isinstance(entries, dict):
            cached = entries.get(entry_key)
            return cached if isinstance(cached, dict) else None
        return None

    async def set_student_analysis_summary_cache(
        self,
        user_id: str,
        course_id: str,
        ...
        user = await self.get_user(user_id) or {"user_id": user_id}
        cache = (
            user.get("student_analysis_summary_cache")
            if isinstance(user.get("student_analysis_summary_cache"), dict)
            else {}
        )
        cache_payload = response if isinstance(response, dict) else summary
        ...
        entry_key = self._analysis_summary_cache_entry_key(cache_key, score_version)
        if entry_key:
            existing_course_cache = (
                cache.get(course_id) if isinstance(cache.get(course_id), dict) else {}
            )
            entries = (
                existing_course_cache.get("entries")
                if isinstance(existing_course_cache.get("entries"), dict)
                else {}
            )
            entries[entry_key] = {
                **cache_payload,
                "cache_key": str(cache_key or "").strip(),
                "score_version": str(score_version or "").strip(),
                "cached_at": cached_at,
            }
            # Keep the profile payload bounded; each course only needs recent variants.
            if len(entries) > 20:
                sorted_keys = sorted(
                    entries,
                    key=lambda key: str(entries[key].get("cached_at") or ""),
                    reverse=True,
                )
                entries = {key: entries[key] for key in sorted_keys[:20]}
            cache[course_id] = {
                "entries": entries,
                "latest_key": entry_key,
                "cached_at": cached_at,
            }
        else:
            cache[course_id] = {**cache_payload, "cached_at": cached_at}
        user["student_analysis_summary_cache"] = cache
        user["updated_at"] = _utcnow()
        return await self._upsert("profiles", user)

    async def invalidate_student_analysis_summary_cache(
        self, user_id: str, course_id: Optional[str] = None
    ) -> bool:
        user = await self.get_user(user_id)
        if not user:
            return True
        cache = user.get("student_analysis_summary_cache")
    ...
    async def get_student_chat_energy_status(self, user_id: str) -> Dict[str, Any]:
        ...
        if not used_thb:
            ...
            if not used_usd:
                used_usd = max(
                    0, int(self._safe_float(usage.get("input_tokens"), 0))
                ) * self._safe_float(
                    self.settings.openrouter_chat_input_cost_per_token_usd,
                    0.00000025,
                ) + max(
                    0, int(self._safe_float(usage.get("output_tokens"), 0))
                ) * self._safe_float(
